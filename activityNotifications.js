...
function activityNotifications(expenditure, d) {
  ...
  for(let i = 0; i <= n - d - 1; i++) {
    let tradingExpenditures = expenditure.slice(i, d + i) 
    let sortedTradingExpenditures = tradingExpenditures.sort((a,b) => a - b) 
    let median; 
    let m = sortedTradingExpenditures.length
    if(m % 2 === 0){
      let first = sortedTradingExpenditures[(m / 2) - 1]
      let second = sortedTradingExpenditures[m / 2]
      median = (first + second) / 2
    } else {
      let centreIndex = Math.floor(m / 2) 
      median = sortedTradingExpenditures[centreIndex]
    }
    let expenditureAfterD = expenditure[d + i]
    if(expenditureAfterD >= (2 * median)) {
      notificationCounter++
    }
  }
